...
export default function ManageCombos({
	...
	const handleToggleCombo = async (combo: ComboWithDetails) => {
		...
		setCombos((prev) => prev.map((c) => (c.id === combo.id ? { ...c, enabled: newEnabledState } : c)));
	...
	const updateModifierQuantity = (dessertId: number, quantity: number) => {
		...
		setFormData((prev) => ({
			...prev,
			items: prev.items.map((item) => (item.dessertId === dessertId ? { ...item, quantity } : item)),
		}));
	};
				...
				<Dialog open={openModal} onOpenChange={handleCloseModal}>
					<DialogContent className="mx-2 max-w-[calc(100vw-1rem)] sm:mx-4 sm:max-w-[calc(100vw-2rem)] md:max-w-lg md:mx-0 max-h-[90vh] overflow-y-auto">
						...
						<form onSubmit={handleSubmit} className="space-y-4">
							...
							<div className="space-y-2">
								<Label htmlFor="overridePrice">Override Price (leave empty to compute)</Label>
								<Input
									id="overridePrice"
									type="number"
									min="0"
							...
							<div className="space-y-2">
								<div className="flex items-center justify-between gap-2">
									...
									{modifiers.length === 0 && (
										<Button
											variant="outline"
											size="sm"
											render={<Link href="/admin/desserts">Create add-ons</Link>}
										></Button>
									)}
								</div>
								...
								{modifiers.length === 0 ? (
									<div className="border rounded-md p-3 text-sm text-muted-foreground">
										No modifier desserts found. In <strong>Admin → Desserts</strong>, create a dessert with Type ={" "}
										<strong>Modifier (Add-on)</strong>, then come back here.
									</div>
								) : (
							...
							<div className="flex gap-2 pt-2">
								...
								<Button type="button" variant="outline" onClick={handleCloseModal} className="ml-auto">
									Cancel
								</Button>
